...
Now that we have defined and built the state and dynamic objects, we can run a simulation. We simulate changes to the koala population over ten timesteps. Runtime will depend on the complexity of the state object and the configuration of the dynamic object.
...
Once a single simulation has been run, we can plot temporally-explicit information.
...
We can also perform multiple simulations. For the koala, we specified five replicates of a ten timestep simulation. To improve computation time, we have opted to use multiple processors by specifying a parameter to the 'future' package in the plan() call.
...
Because a simulation object is comprised of single or multiple replicates, we can subset and plot individual replicates. Here, we plot the population projections for the fifth replicate of the simulation:

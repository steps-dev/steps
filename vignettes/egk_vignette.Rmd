...
We can also perform multiple simulations. For the kangaroo, we specify three replicates of a twenty timestep simulation. The replicates are run sequentially by default, however, to improve computation time, we have included the ability to run all three replicates in parallel - each on a different processor. See ?plan for more information on how to operate in parallel. Note, the verbose argument is not required if simulations are run in parallel as progress bars are only shown in sequential simulations.

```{r, message = FALSE, progress = FALSE, echo = FALSE}

...
v0 <-   c(80,40,10)
dm <- demographic(v0=v0,tmat=tmat,matsd=matsd,estdem=TRUE,time=100,nrep=100)
```

We can now look at how this single population changes over time.
```{r plot_all,fig.width = 9, fig.height = 4, dpi = 300, set_xpd = TRUE}
plot(dm)
```

We can all see the different stages in the population structure change through time.
```{r plot_all,fig.width = 9, fig.height = 4, dpi = 300, set_xpd = TRUE}
plot(dm,mean_pop = FALSE)
```
